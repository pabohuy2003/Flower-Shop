...
public class CartController extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        HttpSession session = request.getSession();
        List<Cart> lstCart = (List<Cart>) session.getAttribute("lstCart");
        String[] orderDetailQuantityStr = request.getParameterValues("orderDetailQuantity");
        for (int i = 0; i < orderDetailQuantityStr.length; i++) {
            lstCart.get(i).setOrderDetailQuantity(Integer.parseInt(orderDetailQuantityStr[i]));
        }
